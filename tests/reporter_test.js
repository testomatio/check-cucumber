...
describe("Reporter", () => {
    ...
    it("should fetch files from server successfully", async () => {
      const mockResponse = {
        files: {
          ...
          "folder/test2.manual.feature":
            "Feature: Test 2\n  Scenario: Another scenario",
        },
        tests: [{ id: "@T123", name: "Test scenario", suite_id: "@S456" }],
        suites: [{ id: "@S456", name: "Test Suite" }],
      };
  ...
  describe("parseLabels", () => {
    ...
    it("should handle label:value format", () => {
      const reporter = new Reporter(API_KEY);
      const result = reporter.parseLabels(
        "severity:high,feature:auth,team:backend",
      );
      expect(result).to.deep.equal([
        "severity:high",
        "feature:auth",
    ...
    it("should handle mixed simple labels and label:value format", () => {
      const reporter = new Reporter(API_KEY);
      const result = reporter.parseLabels(
        "smoke,severity:critical,regression,feature:user_account",
      );
      expect(result).to.deep.equal([
        "smoke",
        "severity:critical",
  ...
  describe("send", () => {
    ...
    it("should include language in payload", async () => {
      let requestBody;
    ...
    describe("labels functionality", () => {
      ...
      it("should add labels to all tests when TESTOMATIO_LABELS is set", async () => {
        ...
        expect(requestBody.tests[0].labels).to.deep.equal([
          "smoke",
          "regression",
        ]);
        expect(requestBody.tests[1]).to.have.property("labels");
        expect(requestBody.tests[1].labels).to.deep.equal([
          "smoke",
          "regression",
        ]);
      });
      ...
      it("should include labels in payload alongside language", async () => {
        ...
        expect(requestBody.files).to.deep.equal({
          "test.feature": "Feature content",
        });
      });
    });
  });
